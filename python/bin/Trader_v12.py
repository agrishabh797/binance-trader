...
def create_limit_order(symbol, position_id, starting_margin, current_margin, side, conn, um_futures_client, position_side):
    ...
    loss = float((50 * current_margin) / 100)
...
def check_current_status_and_update(position_id, conn, um_futures_client, position_side="BOTH"):
    ...
    if current_margin == 0.0:
        ...
        if limit_src_order_id:
            logging.info("Getting Order information from API for Limit Order Id %s", limit_src_order_id)
            response = um_futures_client.query_order(symbol=symbol, orderId=limit_src_order_id)
            logging.info("response_limit: %s", response)
...
def create_position(batch_id, symbol, leverage, each_position_amount, conn, um_futures_client):
    ...
    cursor = conn.cursor()
    position_id_query = "select id, starting_margin, side, position_quantity from positions where symbol = '{}' and position_status = 'OPEN'".format(symbol)
...
def check_and_update_symbols(conn, um_futures_client):
    ...
    for position in exchange_info['symbols']:
        if position['symbol'].endswith('BUSD') and position['status'] == 'TRADING' and position['symbol'] not in ("BTCBUSD", "ETHBUSD", "SOLBUSD"):
